...
readAndSetColor(int clrIdx, char* txt) {
	...
	buf[7] = '\0';
	if(isHexColor(buf)) {
		drw_clr_create(drw, &drw->scheme[clrIdx], buf, baralpha); // baralpha in config.h for alpha
	}
}
...
//	system("cd ~/scripts; ./autostart_blocking.sh");
	system("/home/jiang/.scripts/autostart.sh &");
}
...
setup(void)
{
	...
	scheme[LENGTH(colors)] = drw_scm_create(drw, colors[0], alphas[0], 3); // alpha[0] for alpha
...
updatesystray(void)
{
    ...
    /* XFillRectangle(dpy, systray->win, XCreateGC(dpy, root, 0, tmp), 0, 0, w, bh); */
	XSync(dpy, False);
}
